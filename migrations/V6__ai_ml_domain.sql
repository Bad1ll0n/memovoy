...
DECLARE
  total_pct INTEGER;
BEGIN
  -- Soma sem contar a linha que está a ser inserida/actualizada
  ...
  IF total_pct + NEW.traffic_percentage != 100 THEN
    RAISE EXCEPTION
      'A soma de traffic_percentage das versões activas deve ser 100. Actual sem esta linha: %, a adicionar: %',
      total_pct, NEW.traffic_percentage;
  END IF;
