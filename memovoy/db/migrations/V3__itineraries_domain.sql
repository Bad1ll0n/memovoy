...
CREATE TABLE itinerary_activities (
  ...
  -- Integrações externas: ID do POI em APIs de terceiros
  external_id     TEXT,
  ...
  CONSTRAINT itinerary_activities_external_chk CHECK (
...
COMMENT ON COLUMN itinerary_activities.external_id IS 'ID do POI em API externa. Ex: "ChIJN1t_tDeuEmsRUsoyG83frY4" (Google Places).';
COMMENT ON COLUMN itinerary_activities.external_source IS 'API de origem: "google_places", "getyourguide", "booking", "airbnb".';
  ...
  THEN
    ...
    SET
      ...
      countries_visited = CASE
        ...
        END,
      total_countries = COALESCE(array_length(
        CASE
          WHEN NEW.country_code = ANY(COALESCE(countries_visited, ARRAY[]::CHAR(2)[]))
            THEN countries_visited
            ELSE array_append(COALESCE(countries_visited, ARRAY[]::CHAR(2)[]), NEW.country_code)
