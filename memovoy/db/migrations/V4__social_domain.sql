...
-- Feed ordenado por popularidade (inclui likes_count para evitar heap fetch)
CREATE INDEX idx_posts_user_popular
  ON posts(user_id, likes_count DESC, created_at DESC)
  WHERE deleted_at IS NULL AND is_hidden = false;
...
-- Lookup "já dei like neste post/comentário?" — query mais frequente do feed
CREATE INDEX idx_reactions_target
  ON reactions(target_id, target_type, user_id);
